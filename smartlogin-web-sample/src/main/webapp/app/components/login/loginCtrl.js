function LoginCtrl($scope, $http, SQRLLoginService, $state, $rootScope, constants) {
    ...
    function sqrlSetup() {
        ...
        $scope.sqrl.watcher.unregisterLogin = $scope.$watch(function () {
            return SQRLLoginService.watchLoginPreparation(constants.IDP_CLIENT_ID, loginSucceededCallback);
        }, function (newValue) {
            if (newValue) {
    ...
    $scope.login = function (username, password, successCallback) {
        $http(
            {
                method: 'POST',
                url: constants.IDP_HOST + '/realms/' + constants.IDP_REALM + '/protocol/openid-connect/token',
                data: {
                    'username': username,
                    'password': password,
                    'grant_type': 'password',
